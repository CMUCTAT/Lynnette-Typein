	...
	function MathUtil() {
		...
		**/
		...
		this.isCancelable = function (t, scannedTreeExp) {
			...
			let likeTerms = tStr.includes('x') ? scannedTreeExp.eqData.varOperands : scannedTreeExp.eqData.constOperands;
		...
		this.getSubtractableTerms = function (subtractFrom, otherSide) {
			...
			if (!addedVars && varTerms.length === 1 && varTerms[0].isSimple && varTerms[0].coeff === -1) {
				//special case: always allow moving x term with coeff of -1
				subtractable.push(varTerms[0]);
			}
