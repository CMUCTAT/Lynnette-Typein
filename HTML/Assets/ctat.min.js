...
function(a){return p.teamMembers[a]&&0<p.teamMembers[a].subscribed};this.allEssentialPresent=function(){var a=CTATConfiguration.get("collaborators"),a=a&&a.split(/,/)||[];return 1>a.length||1==a.length&&a[0]==CTATConfiguration.get("user_guid")&&CTATCommShell.commShell.getGotProblemRestoreEnd()||a.every(function(a){return a&&(!CTATCommLibrary.teamMemberEssential(a)||p.teamMembers[a]&&0<p.teamMembers[a].subscribed)})||CTATConfiguration.get("instructor_mode")&&a.every(function(a){return!a||!p.teamMembers[a]||
0>p.teamMembers[a].subscribed})};this.interpretCollab=function(a){p.log("collaborate","Interpret collab:",a.command,a);var c=null,d=CTATConfiguration.get("user_guid");if("tutorready"==a.command&&a.username!=d){var b=CTATConfiguration.get("collaborators"),e="";p.log("collaborate","interpretCollab",b,"teamMembers",p.teamMembers,p.tutorbeginListener);if(CTATConfiguration.get("instructor_mode")&&CTATCommLibrary.teamMemberEssential(a.username)&&p.tutorbeginListener){p.tutorbeginListener(a);return}if(CTATConfiguration.get("instructor_mode")&&
p.teamMembers[d]&&0<p.teamMembers[d].subscribed&&(!p.teamMembers[a.username]||0>p.teamMembers[a.username].subscribed)&&CTATCommLibrary.teamMemberEssential(a.username)){a=CTATGlobals.languageManager.filterString("YOUR_STUDENT")+(a.display_name||a.from)+" ";"function"==typeof CTATLMS.exitTutor?(a+=CTATGlobals.languageManager.filterString("EXIT_BECAUSE_COACHED_STUDENT_JOINED_OK"),p.log("collaborate","OKScrimUp("+a+")",new Date,CTATLMS.exitTutor),CTATScrim.scrim.OKScrimUp(a,CTATLMS.exitTutor.bind(this,
"force"))):(a+=CTATGlobals.languageManager.filterString("EXIT_BECAUSE_COACHED_STUDENT_JOINED_X"),p.log("collaborate","scrimUp("+a+")",new Date,typeof CTATLMS.exitTutor),CTATScrim.scrim.scrimUp(a,y));return}for(e in a.teamMembers||{})b=a.teamMembers[e],e!=d&&(p.teamMembers[e]=b),0>b.subscribed&&p.log("collaborate","collaborator quit:",b,new Date);p.sendTutorReady(p.teamMembers);document.dispatchEvent(new CustomEvent("teamchange",{detail:p.teamMembers}))}else"tutordone"==a.command?(p.log("collaborate",
"tutordone listener calling tutordoneListener"),p.tutordoneListener&&p.tutordoneListener(a)):"message"==a.command?a.from!=d?c=a.data:ctatdebug("Own message echoed: "+a.data):"unsubscribed"==a.command?(e=p.teamMembers[a.from]||(p.teamMembers[a.from]={}),e.username=a.from,e.subscribed=-1,e=CTATGlobals.languageManager.filterString("YOUR_COLLABORATOR")+(a.display_name||a.from||"")+" ",a.from==d?p.log("collaborate","interpretCollab: own unsubscribe received",a):CTATCommLibrary.teamMemberEssential(a.from)?
"function"==typeof CTATLMS.exitTutor?(d=e+CTATGlobals.languageManager.filterString("COLLABORATOR_QUIT_CLICK_OK"),p.log("collaborate","OKScrimUp("+d+")",new Date,CTATLMS.exitTutor),CTATScrim.scrim.OKScrimUp(d,CTATLMS.exitTutor.bind(this,"force"))):(d=e+CTATGlobals.languageManager.filterString("COLLABORATOR_QUIT_CLICK_X"),p.log("collaborate","scrimUp("+d+")",new Date,typeof CTATLMS.exitTutor),CTATScrim.scrim.scrimUp(d,y)):p.log("collaborate","interpretCollab: optional member unsubscribed",a),document.dispatchEvent(new CustomEvent("teamchange",
{detail:p.teamMembers}))):"subscribed"==a.command?a.team_members?(p.log("collaborate","subscribed from "+a.from+": teamMembers",p.teamMembers,"data.team_members",a.team_members,"allEssentialPresent()",p.allEssentialPresent()),(!CTATConfiguration.get("instructor_mode")||p.allEssentialPresent())&&p.commReady&&p.commReady.dispatchEvent(new CustomEvent("subscribed",{detail:a})),a.from!=d&&p.sendTutorReady(p.teamMembers)):console.err("collaborate","interpretCollab: subscribed with team_members property missing",
a):"requestexit"==a.command&&(d=a.data,p.log("collaborate","OKscrimUp or scrimUp("+d+")",new Date,typeof CTATLMS.exitTutor),"function"==typeof CTATLMS.exitTutor?CTATScrim.scrim.OKScrimUp(d,CTATLMS.exitTutor.bind(this,!1)):CTATScrim.scrim.scrimUp(d,y));"function"==typeof document.dispatchEvent&&document.dispatchEvent(new CustomEvent("receiveFromCollaborator",{detail:a}));return c};this.setSocketType=function(a){q=a;CTAT.ToolTutor.commReadyPromise=CTATLMS.is.TutorShop()?new Promise(function(a,c){(p.commReady=
new EventTarget).addEventListener("subscribed",function(c){p.log("collaborate","commReady listener received",c,"\n calling resolve",a);a(c)});p.log("collaborate","commReadyPromise defined listener commReady",p.commReady)}):Promise.resolve("no channel needed");if(p.hasWebSocket()||p.hasPostMessage()||CTATConfiguration.get("post_tracer_output")||CTATLMS.is.TutorShop())p.log("commlibrary","setSocketType: connecting message listener",a),$(window).on("message",function(a){if(a.originalEvent.origin===window.location.origin&&
(a=a.originalEvent?a.originalEvent.data:null,p.log("commlibrary","message from "+a.from+":",a),a))if(d){if(a=p.interpretCollab(a))d.processMessage(a),p.hasPostMessage()||CTAT.ToolTutor.sendToTutor(a)}else console.log("No http handler defined for websocket listener.")})};this.getSocketType=function(){return q};this.setScrimWait=function(a){y=a};this.hasSecureSocket=function(){return/https/i.test(q)||/wss/i.test(q)};this.hasJavaScriptConnection=function(){return/javascript/i.test(q)};this.hasWebSocket=
function(){return/websocket/i.test(q)};this.hasPostMessage=function(){return/postmessage/i.test(q)};this.assignHandler=function(a){d=a};this.assignMessageListener=function(a){n=a};this.encodeVariables=function(a){p.ctatdebug("encodeVariables ()");for(var c="",d=0;d<a.length;d++){var b=a[d];0<d&&(c+="&");c+=b.name;c+="=";c+=encodeURIComponent(b.value)}return c};this.encodeVariablesOLI=function(a){p.ctatdebug("encodeVariablesOLI ()");for(var c="",d=0;d<a.length;d++){var b=a[d];0<d&&(c+="&");c+=b.name;
c+="=";c+=encodeURIComponent(b.value)}return c};this.createConnection=function(d,b,f){p.ctatdebug("createConnection () canBeWebSocket "+f);f="undefined"==typeof f?!0:!1!==f;if(!b)return new CTATConnection(d);if(f&&p.hasWebSocket())return f=p.findWSConnection(b),f||(f=new CTATWSConnection(d),f.setID(e),f.setURL(b),f.assignReceiveFunction(p.processWSReply),f.assignCloseFunction(p.processWSClose),c.push(f),e++),f;f=new CTATConnection(d);f.setID(e);f.setURL(b);f.assignReceiveFunction(p.processReply);
c.push(f);e++;return a=f};this.findWSConnection=function(a){ctatdebug("findWSConnection( "+a+" )");a=p.editSocketURLForHTTPS(a);for(var d=0;d<c.length;d++){var b=c[d];ctatdebug("checking: socketType = "+b.getSocketType()+" URL = "+b.getURL());if("ws"==b.getSocketType()&&b.getURL()==a)return b}return null};this.startBundle=function(){p.ctatdebug("startBundle ()");!1===useBundling?p.ctatdebug("Not using bundling, bump"):(bundleFormatter='<?xml version="1.0" encoding="UTF-8"?><message><verb/><properties><MessageType>MessageBundle</MessageType><messages>',
inBundle=!0)};this.endBundle=function(){p.ctatdebug("endBundle ()");!1===useBundling?p.ctatdebug("Not using bundling, bump"):(inBundle=!1,bundleFormatter+="</messages></properties></message>",p.sendXML(bundleFormatter))};this.setFixedURL=function(a){f=a};this.getURL=function(){p.ctatdebug("getURL ()");if(""!==f)return p.ctatdebug("Returning fixedURL: "+f),f;var a=flashVars.getRawFlashVars(),c="http://";p.hasSecureSocket()&&(c="https://");c=c+a.remoteSocketURL+":"+a.remoteSocketPort;a.remoteSocketURL&&
-1!=a.remoteSocketURL.indexOf("http")&&(c=a.remoteSocketURL+":"+a.remoteSocketPort);return c};this.sendXML=function(a,c){p.ctatdebug("Sending: "+a);p.ctatdebug("sendXML ("+a+")");d.updateLastActionTimestamp&&d.updateLastActionTimestamp();if(!0===useBundling&&!0===inBundle)p.ctatdebug("Bundling ..."),bundleFormatter+=a;else{bundleFormatter=a;flashVars.getRawFlashVars();a=p.getURL();var b=bundleFormatter;-1==bundleFormatter.indexOf("<?xml")&&(b='<?xml version="1.0" encoding="UTF-8"?>'+bundleFormatter);
p.getUseCommSettings()&&p.hasJavaScriptConnection()?(p.sendToToolsOrCollaborators(a,b,c),CTATConfiguration.get("noLocalTrace")||CTAT.ToolTutor.sendToTutor(b)):p.send_post(a,b)}};this.sendToToolsOrCollaborators=function(a,c,d){return!p.hasWebSocket()||"ctatAsListener"!=CTATConfiguration.get("mode")&&"undefined"!=typeof d&&!1===d?!1:(p.send_post(a,c),!0)};this.sendXMLNoBundle=function(a){p.ctatdebug("sendXMLNoBundle ("+a+")");bundleFormatter=a;d.updateLastActionTimestamp&&d.updateLastActionTimestamp();
var c=p.getURL(),b=a;-1==a.indexOf("<?xml")&&(b='<?xml version="1.0" encoding="UTF-8"?>'+a);flashVars.getRawFlashVars();p.getUseCommSettings()&&p.hasJavaScriptConnection()?(p.sendToToolsOrCollaborators(c,b),CTAT.ToolTutor.sendToTutor(b)):p.send_post(c,b)};this.sendXMLURL=function(a,c,b){ctatdebug("sendXMLURL ("+c+", "+b+")");var e=a;-1==a.indexOf("<?xml")&&(e='<?xml version="1.0" encoding="UTF-8"?>'+a);flashVars.getRawFlashVars();p.ctatdebug("Sending: "+e);d.updateLastActionTimestamp&&d.updateLastActionTimestamp();
p.getUseCommSettings()&&p.hasJavaScriptConnection()?(p.sendToToolsOrCollaborators(c,e),b||CTAT.ToolTutor.sendToTutor(e)):p.send_post(c,e)};this.send_post_variables=function(a,c){p.ctatdebug("send_post_variables ("+a+")");flashVars.getRawFlashVars();c=!1===useOLIEncoding?p.encodeVariables(c):p.encodeVariablesOLI(c);this.ctatdebug("Sending: "+c);d.updateLastActionTimestamp&&d.updateLastActionTimestamp();if(!0===CTATGlobals.CommDisabled)return p.ctatdebug("Communications globally disabled, please check your settings"),
Promise.reject("Communications globally disabled, please check your settings");var b=p.createConnection(CTATConfiguration.getRawFlashVars(),a,!1);b.setContentType("application/x-www-form-urlencoded");p.ctatdebug("CTATCommLibrary.send_post_variables() data to send:\n  "+c);null!==n&&n.processOutgoing(c);b.setURL(a);b.setData(c);b.assignReceiveFunction(p.processReply);b.send();return b};this.formPostMessage=function(a){return{command:"message",id:CTATCommLibrary.makeTeamID(),user_guid:CTATConfiguration.get("user_guid"),
data:a}};this.send_post=function(a,c){ctatdebug("send_post ("+a+")");if(CTATConfiguration.get("editorMode")||!0===CTATGlobals.CommDisabled)p.ctatdebug("Communications globally disabled, please check your settings");else{ctatdebug("Outgoing on wire, "+(p.hasPostMessage()?"to post":"no post")+"; data "+c);var d=flashVars.getRawFlashVars(),b=a;if(p.hasPostMessage())c=p.formPostMessage(c),c.dest="sendToTutor",p.log("commlibrary","send_post haspostmessage sending sendToTutor to parent",c),window.parent.postMessage(c,
location.origin);else{if(p.hasWebSocket()){if(CTATCommShell.commShell.hasTutorShopCable()){c=p.formPostMessage(c);p.log("commlibrary","send_post hasWebSocket not sendToTutor to parent",c);window.parent.postMessage(c,location.origin);return}a&&(ctatdebug("opening websocket connection: url "+a),b=a.replace(/https?:/,"ws:"),b=p.editSocketURLForHTTPS(b),ctatdebug("opening websocket connection to "+b))}a=p.createConnection(d,b);a.setData(c);null!==n&&n.processOutgoing(c);a.send()}}};this.processReply=
function(a,b,e){p.ctatdebug("processReply ("+c.length+","+a+", ...)");"undefined"==typeof e&&"function"==typeof p.getHTTPObject&&p.getHTTPObject();b=0;a=[];var f;for(f=0;f<c.length;f++){var g=c[f];if(g&&"function"==typeof g.getHTTPObject){var l=g.getHTTPObject();p.ctatdebug("Testing connection entry["+f+"]: readyState & status: "+(l?l.readyState+" & "+l.status:"null")+", consumed: "+g.getConsumed());l&&(4==l.readyState&&!1===g.getConsumed()?(p.ctatdebug("Investigating request response: "+b+" -> "+
l.status+", for: "+g.getURL()),e=!1,0===l.status&&(e=!0,p.ctatdebug("Received message (status 0): ("+l.responseText+"), status: "+l.status),h&&CTATScrim.scrim.errorScrimUp(CTATGlobals.languageManager.filterString(u))),408==l.status&&(e=!0,p.ctatdebug("Received message (status 408): "+l.responseText),CTATScrim.scrim.scrimDown()),502==l.status&&(e=!0,p.ctatdebug("Received message (status 502): "+l.responseText),h&&CTATScrim.scrim.errorScrimUp(CTATGlobals.languageManager.filterString(ERROR_502))),200==
l.status?(e=!0,p.ctatdebug("Processing 200 response ..."),null!==d?(a.push(l.responseText),null!==n&&n.processIncoming(l.responseText)):p.ctatdebug("Error: httphandler is null, can't process response!")):d&&d.processError&&(e=!0,p.ctatdebug("Processing non-200 response, status "+l.status),d.processError(l.status,l.responseText)),!1===e&&p.ctatdebug("Error: status not handled for: "+l.status),p.ctatdebug("Marking connection as consumed ..."),g.setConsumed(!0)):(0===l.readyState&&p.ctatdebug("Received message (status 0, request not initialized)"),
1===l.readyState&&p.ctatdebug("Received message (status 1, server connection established)"),2===l.readyState&&p.ctatdebug("Received message (status 2, request received)"),3===l.readyState&&p.ctatdebug("Received message (status 3, processing request)")),b++)}}p.cleanup();for(b=0;b<a.length;b++)p.ctatdebug("Processing incoming message: "+b),e=a[b],-1!=e.indexOf("status=success")?p.ctatdebug("Info: logging success message received, not propagating to message handler"):d&&d.processMessage&&(p.ctatdebug("Processing incoming message: "+
e),d.processMessage(e))};this.processWSReply=function(a){p.ctatdebug("processWSReply() length "+(null===a?null:a.toString().length));"string"==typeof a&&(null!==n&&n.processIncoming(a),-1!=a.indexOf("status=success")?p.ctatdebug("processWSReply() logging success message received, not propagating to message handler: "+a):(p.ctatdebug("Processing incoming message: "+a),d.processMessage(a)))};this.processWSClose=function(a){p.ctatdebug("processWSClose("+a+")");a instanceof CloseEvent&&p.ctatdebug("CloseEvent: code "+
a.code+", reason "+a.reason+", wasClean "+a.wasClean);CTATLMS.is.Authoring()||CTATLMS.is.ToolsListener()?CTATScrim.scrim.scrimUp(CTATGlobals.languageManager.filterString("AUTHORPLEASECLOSE")):CTATScrim.scrim.handleTSDisconnect()};this.cleanup=function(){p.ctatdebug("cleanup ()");p.ctatdebug("checkEntries() httprequests.length "+c.length);for(var a=0,d=c.length-1;0<=d;d--){var b=c[d];if(b)if(!0===b.getConsumed())p.ctatdebug("checkEntries() removing["+d+"] ID: "+b.getID()),c.splice(d,1),a++;else if("function"==
typeof b.getHTTPObject){var e=b.getHTTPObject();p.ctatdebug("checkEntries() checking["+d+"], readyState: "+(e?e.readyState:"null")+", consumed: "+b.getConsumed())}}p.ctatdebug("Removed "+a+" entries")};this.retrieveProblemFile=function(a,c,d,b,e,f){"function"==typeof CTATScrim&&CTATScrim.scrim&&h&&CTATScrim.scrim.waitScrimUp();var l=/\.([A-z]*)([?].*)?$/.exec(a);l&&l[1]&&"nools"===l[1].toLowerCase()?(console.log("got a nools file:",a),d.processNools(a,b,e,f)):(console.log("got an xml file, maybe:",
a),p.retrieveXMLFile(a,c,d,b,e,f))};this.getFileRequest=function(a,c,d){var b=-1!=(location.href?location.href:a).indexOf("file://")?!0:!1;location.href||p.ctatdebug("We can't check location.href in the current configuration");if(b)d=(new CTATTutorMessageBuilder).createErrorMessage("","Unable to load: "+a+" - You are trying to use the file:// protocol, which is not allowed in this browser."),p.ctatdebug("onerror GET for fileName "+d),CTAT.ToolTutor.sendToInterface(d,!0);else return v=a,a=new CTATConnection,
a.assignReceiveFunction(function(a,d,b){"undefined"==typeof b&&(b="function"==typeof p.getHTTPObject&&p.getHTTPObject());p.ctatdebug("onready... GET for xmlFile xmlhttp "+b+", .readyState & .status "+(b?b.readyState+" & "+b.status:""));b&&4!=b.readyState||(b&&200==b.status&&c(b),b&&404==b.status&&(a=(new CTATTutorMessageBuilder).createErrorMessage("Unable to download file","("+v+") not found"),p.ctatdebug("Error loading xmlFile "+v+": "+a),CTAT.ToolTutor.sendToInterface(a,!0)))}),a.assignFailFunction(d),
a};this.retrieveXMLFile=function(a,c,d,b,e,f){p.ctatdebug("retrieveXMLFile ("+a+")");c=p.getFileRequest(a,function(a){a.responseXML?(p.ctatdebug("parsing brd xml using something else"),a=a.responseXML.documentElement):(p.ctatdebug("parsing brd xml using node"),a=(new CTATXML).parseXML(a.responseText));if(null===a){var c=(new CTATTutorMessageBuilder).createErrorMessage("Error: empty or null xmlFile "+v);p.ctatdebug("Error parsing xmlFile "+v+": "+c);CTAT.ToolTutor.sendToInterface(c,!0)}d.processXML(a,
b,e,f)},function(){var a=(new CTATTutorMessageBuilder).createErrorMessage("",CTATGlobals.languageManager.getString("UNABLE_TO_LOAD")+v+CTATGlobals.languageManager.getString("FILE_IS_MISSING"));p.ctatdebug("onerror GET for xmlFile "+a);CTAT.ToolTutor.sendToInterface(a,!0);d.processXML(null,b,e)});c.setMethod("GET");c.setURL(a);c.setContentType("text/plain");c.send()};this.retrieveJSONFile=function(a,c){p.ctatdebug("retrieveJSONFile ("+a+")");var d=p.getFileRequest(a,function(){4==d.readyState&&200==
d.status&&(p.ctatdebug("Successfully retrieved file: "+a),c(JSON.parse(d.responseText)))},function(){var a=(new CTATTutorMessageBuilder).createErrorMessage("","Unable to load: "+v+" - Either you are trying to use the  file:// protocol, there is a firewall between the tutor and BRD, or the BRD is on a different domain and permission to retrieve data from that server is denied.");p.ctatdebug("onerror GET for xmlFile "+a);CTAT.ToolTutor.sendToInterface(a,!0)});d.open("GET",a,!0);d.setRequestHeader("Content-type",
"text/plain");d.send()};this.retrieveFile=function(a,c){p.ctatdebug("retrieveFile ("+a+")");var d=p.getFileRequest(a,function(){4==d.readyState&&200==d.status&&(p.ctatdebug("Successfully retrieved file: "+a),c(d.responseText))},function(){var a=(new CTATTutorMessageBuilder).createErrorMessage("","Unable to load: "+v+" - Either you are trying to use the  file:// protocol, there is a firewall between the tutor and BRD, or the BRD is on a different domain and permission to retrieve data from that server is denied.");
p.ctatdebug("onerror GET for xmlFile "+a);CTAT.ToolTutor.sendToInterface(a,!0)});d.open("GET",a,!0);d.setRequestHeader("Content-type","text/plain");d.send()};this.then=function(c,d){return a&&"function"==typeof a.then?a.then(c,d):Promise.resolve("CTATCommLibrary.then() not supported").then(c,d)}};CTATCommLibrary.setAuthenticityToken=function(b){Array.isArray(b)&&(b=0<b.length?b[0]:"");b&&(CTATCommLibrary.authenticityToken=decodeURIComponent(b))};CTATCommLibrary.getAuthenticityToken=function(b){return CTATCommLibrary.authenticityToken};
